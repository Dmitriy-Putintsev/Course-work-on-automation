...
public class DataHelper {
    ...
    @SneakyThrows
    public static OrderDetailsInfo getCreditRequestInfo() {
        var runner = new QueryRunner();
        var creditRequestInfo = "SELECT * FROM credit_request_entity WHERE created = (SELECT MAX(created) FROM credit_request_entity);";

        try (var connection = DriverManager.getConnection(
                datasource, "admin", "qwerty")) {
            return runner.query(connection, creditRequestInfo, new BeanHandler<>(OrderDetailsInfo.class));
        }
    }

    @SneakyThrows
    public static OrderDetailsInfo getPaymentInfo() {
        var runner = new QueryRunner();
        var paymentInfo = "SELECT * FROM payment_entity WHERE created = (SELECT MAX(created) FROM payment_entity);";

        try (var connection = DriverManager.getConnection(
                datasource, "admin", "qwerty")) {
            return runner.query(connection, paymentInfo, new BeanHandler<>(OrderDetailsInfo.class));
        }
    }

    @SneakyThrows
    public static OrderDetailsInfo getOrderInfo() {
        var runner = new QueryRunner();
        var orderInfo = "SELECT * FROM order_entity WHERE created = (SELECT MAX(created) FROM order_entity);";

        try (var connection = DriverManager.getConnection(
                datasource, "admin", "qwerty")) {
            return runner.query(connection, orderInfo, new BeanHandler<>(OrderDetailsInfo.class));
        }
    }
}
